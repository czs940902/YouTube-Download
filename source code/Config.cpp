...
void initialize() {
    ...
    if(Updater::Config::check(dlconfigPath)) loadConfig(dlconfigPath);
    else                                     dlconfig = defaultConfig;
    if(Updater::Custom::check(dlcustomPath)) loadCustom(dlcustomPath);
    else                                     dlcustom = defaultCustom;
...
bool saveConfig(const std::string& filename) {
    ...
    FileRW::writeInt(file, CONFIG_VERSION);
...
bool loadConfig(const std::string& filename) {
    ...
    if(!file.is_open()) return false;
    int dlconfigVersion;
    FileRW::readInt(file, dlconfigVersion);
    for(Config& config : dlconfig)
