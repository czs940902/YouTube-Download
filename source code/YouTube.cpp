...
YouTube::YouTube(const Config& temp)
: config(temp) {
...
void YouTube::modeA() {
    ...
    if(!getURLFromInput())  return;
    if(!getNameFromInput()) return;
    if(!setFileName())      return;
    if(!getAudioFile())     return;
    if(!convertAudio())     return;
...
void YouTube::modeV() {
    ...
    if(!setFileName())      return;
    if(!getAudioFile())     return;
    if(!getVideoFile())     return;
    if(!convertVideo())     return;
    std::cout << "Download completed!\n\n";
}
void YouTube::modeT() {
