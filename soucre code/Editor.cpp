...
void editor() {
    while(true) {
        ...
        description << "Default edited ... " << (Config::Check::isConfigDefault() ? "NO" : "YES") << "\n"
                    << "Custom mode ...... " << (Config::Check::isCustomDefault() ? "NO" : "YES") << "\n";
        operations  << "[0]\tExit\n"
...
namespace Operation {
bool exit() {
    if(!Config::Check::isConfigEdited() && !Config::Check::isCustomEdited()) return true;
    std::stringstream description, operations;
...
void createCustom() {
    ...
    while(true) {
        ...
        if(Config::Check::searchCustom(tempName) < 0) { temp.modeName = tempName; break; }
...
void restoreDefault(Config0& config) {
    if(Editor::UI::yesNo("Reset to default?\n")) {
        int tempModeInt = config.modeInt;
        std::string tempName = config.modeName;
        config.reset();
        config.initialize(tempModeInt);
        config.modeName = tempName;
        Editor::UI::alert("Reset succeed!");
    }
}
int generateOperation(int index, std::unordered_map<int, int>& modeMap, Config0& config, std::stringstream& operations) {
...
void operationSelector(int operation, Config0& config) {
    switch(operation) {
...
void editTxtURL(Config0& config, const std::string& str1, const std::string& str2) {
    std::stringstream description;
    description << str1 << " ... " << "\"" << config.txtURL << "\"\n";
    if(Editor::UI::yesNo(description.str()))
        config.setTxtURL(Editor::UI::line(description.str(), str2));
}
void editTxtName(Config0& config, const std::string& str1, const std::string& str2) {
    std::stringstream description;
